...
contract("MCCUserToken", accounts => {
    ...
    const userEmail1 = "user1@mcc.com"
    ...
    it("should revert when the sender in not MCC", async () => {
        ...
        // WHEN
        // THEN
        await expectRevert(this.contractInstance.createUser(email, { from: sender }), "Ownable: caller is not the owner");
    });
    it("should revert when the email has alredy been used", async () => {
        ...
        await this.contractInstance.createUser(email, { from: sender });

        // WHEN
        // the same email is used for the 2nd time
        // THEN
        await expectRevert(this.contractInstance.createUser(email, { from: sender }), "Email already used");
    });
    it("should create an User when the sender is MCC, and emit event `NewUser`", async () => {
        ...
        // WHEN
        const receipt = await this.contractInstance.createUser(email, { from: sender });
